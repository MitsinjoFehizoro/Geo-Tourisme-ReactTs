...
import LoadingTemoinCard from "./Loading-temoin-card";
import { useInformation } from "../../hooks/useInformation";

const Temoin: FunctionComponent = () => {
    const { information, stateGetInformation } = useInformation()
    return (
        <motion.section
            initial='offscreen'
            ...
            <motion.div variants={variantsStandard} className="w-full flex flex-wrap justify-around">
                {
                    stateGetInformation.isLoading ? (
                        ...
                        information && information.witnesses.length >= 2 && (
                            information.witnesses.map((witness, index) =>
                                <TemoinCard key={index} witness={witness} />
                            )
                        )
                    )
                }
